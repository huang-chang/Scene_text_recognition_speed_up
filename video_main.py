        ...
        if cap.isOpened():
            ...
            while ret and img is not None:
                if frame_id % 10 == 0:
                    ...
                    t0 = time.time()
                    total_image, total_width, img_framed, detect_time, detect_number= self.image_process.text_region_run(img_cut)
                    t1 = time.time()
                    
                    print('detect_number: {} detect_time: {} frame_id: {}'.format(detect_number, t1-t0, frame_id))
                    if len(total_width) > 0:
                        cptn_time.append([frame_id, t1-t0])
                        for i in range(len(total_width)):
                    ...
                    img_drawed_rgb = cv2.cvtColor(img_framed, cv2.COLOR_RGB2BGR)
...
#                    break
        ...
        for item in video_text_width:
            t0 = time.time()
            result = self.image_process.densenet_recognition_run(video_text_region[item[-1]][0])
            t1 = time.time()
            #print('densenet time: {}'.format(t1-t0))
            densenet_time.append([item[-2], t1-t0])
            video_text_results.append([item[-2], result])
            frame_set.add(item[-2])
        ...
        temp_result = ''
        temp_time = 0.0
        new_video_text_results = []
        new_densenet_time = []
        
        for item in frame_list:
            for sub_index, sub_item in enumerate(video_text_results):
                if item == sub_item[0]:
                    temp_result = temp_result + sub_item[1]
                    temp_time = temp_time + densenet_time[sub_index][1]
            new_video_text_results.append([item, temp_result])
            new_densenet_time.append([item, temp_time])
            temp_result = ''
            temp_time = 0.0
            
        with open(save_label_path,'w') as f:
            for item in new_video_text_results:
                current_image_file = os.path.join(save_path,'{}.jpg'.format(item[0]))
                f.write('{}卍{}卍{}卍{}\n'.format(item[0],item[1],current_image_file,0))
        total_time = []
        for index, item in enumerate(cptn_time):
            print('ocr_time, {}:{}'.format(item[0], item[1]+new_densenet_time[index][1]))
            total_time.append(item[1]+new_densenet_time[index][1])
        print('ocr_mean_time: {}/frame'.format(np.mean(total_time)))
...
def parse_arguments(argv):
    ...
    parse.add_argument('--result_path', type = str, help = 'path to result for test videos',default = 'scene_text_recogition')
